"""
SecureEye Intelligence AI - Version 10.0 (High-Impact Summary Engine)
...
async def scrape_link(url: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=20, follow_redirects=True) as client:
            headers = {"User-Agent": "SecureEye-Analyst-Bot/4.0"}
...
async def get_ai_summary(content: str) -> str:
    ...
    if settings.GEMINI_API_KEY:
        try:
            ...
            prompt = f"""
            You are a senior cybersecurity analyst at SecureEye. 
            Analyze the following security content and provide a high-impact, professional summary.
            
            STRUCTURE:
            1. Title: 🛡️ SECUREEYE EXECUTIVE SUMMARY: [CVE-ID or Threat Name]
    ...
    report = []
    report.append(f"🛡️ SECUREEYE EXECUTIVE SUMMARY: {cve_id}")
    report.append("="*55)
    report.append("")
    ...
    report.append("")
    report.append("-" * 55)
    report.append(f"Analyst Note: Priority Remediation Required | Generated by SecureEye AI")
